...
int sum (int count,...) {   
    va_list args;   // 使用va_list型態變數來循環可變參數list
    va_start(args,count);   // 使用va_start將參數逐一遞增,va_start初始化變數args

    int result = 0;
    for(int i = 0;i<count;i++){
        result += va_arg(args,int);
    }
    va_end(args);   //並完成之後va_end結束可變參數的獲取
    return result;
}

int min(int count,...) {
    va_list args;   //接收一定數量的整數，並返回最小值
    va_start(args,count);
